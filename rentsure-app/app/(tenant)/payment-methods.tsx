/**
 * Tenant Payment Methods Screen.
 *
 * Allows tenants to manage their payment methods — Visa/Mastercard and
 * mobile money numbers used for Paystack quick checkouts.
 */
...
interface PaymentMethod {
  ...
  label: string;
  accountName: string;
  accountNumber: string; // Used for card number or phone number
  provider: string; // e.g., 'Visa', 'Mastercard', 'MTN MoMo'
  isDefault: boolean;
}
...
export default function TenantPaymentMethodsScreen() {
  ...
  // Form state
  const [formType, setFormType] = useState<'CARD' | 'MOMO'>('CARD');
  ...
  const loadMethods = () => {
    if (!db.paymentMethods) {
      (db as any).paymentMethods = [];
    }
    const myMethods = (db as any).paymentMethods.filter(
      (m: any) => m.tenantId === user?.id
    );
  ...
  const handleSave = async () => {
    ...
    if (editingMethod) {
      // Update existing
      const idx = (db as any).paymentMethods.findIndex((m: any) => m.id === editingMethod.id);
      if (idx >= 0) {
        (db as any).paymentMethods[idx] = {
          ...(db as any).paymentMethods[idx],
          type: formType,
          provider: formProvider,
          accountName: formAccountName,
          accountNumber: formAccountNumber,
          label: formType === 'CARD' ? `•••• ${formAccountNumber.slice(-4)}` : `${formProvider} - ${formAccountNumber}`,
        };
      }
    } else {
      // Add new
      const isFirst = (db as any).paymentMethods.filter((m: any) => m.tenantId === user?.id).length === 0;
      (db as any).paymentMethods.push({
        id: generateId(),
        tenantId: user?.id,
        type: formType,
        provider: formProvider,
  ...
  const handleDelete = (method: PaymentMethod) => {
    Alert.alert('Remove Payment Method', `Delete ${method.label}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete',
        style: 'destructive',
        onPress: async () => {
          (db as any).paymentMethods = (db as any).paymentMethods.filter(
  ...
  const handleSetDefault = async (method: PaymentMethod) => {
    (db as any).paymentMethods.forEach((m: any) => {
      if (m.tenantId === user?.id) {
  ...
  const CARD_PROVIDERS = ['Visa', 'Mastercard'];
  ...
  return (
    <Screen noPadding style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {methods.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySub}>
              Add a card or mobile money number for quicker checkout.
            </Text>
          </View>
        ) : (
                ...
                <View style={styles.methodHeader}>
                  ...
                  <View style={styles.methodInfo}>
                    ...
                    <Text style={styles.methodNumber}>
                      {method.type === 'CARD' ? `•••• •••• •••• ${method.accountNumber.slice(-4)}` : method.accountNumber}
                    </Text>
                  </View>
                  {method.isDefault && (
        ...
        <Button title="Add Payment Method" onPress={openAdd} style={styles.addBtn} />
      </ScrollView>
          ...
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            ...
            <ScrollView style={styles.modalBody}>
              ...
              <View style={styles.typeRow}>
                <TouchableOpacity
                  style={[styles.typeBtn, formType === 'CARD' && styles.typeBtnActive]}
                  onPress={() => {
                    setFormType('CARD');
                    setFormProvider('');
                  }}
                >
                  <Ionicons
                    name="card-outline"
                    size={18}
                    color={formType === 'CARD' ? '#FFF' : colors.text}
                  />
                  <Text
                    style={[
                      styles.typeBtnText,
                      formType === 'CARD' && styles.typeBtnTextActive,
                    ]}
                  >
                    Credit / Debit Card
                  </Text>
              ...
              <Text style={styles.fieldLabel}>
                {formType === 'CARD' ? 'Card Type' : 'Network'}
              </Text>
              <View style={styles.providerList}>
                {(formType === 'CARD' ? CARD_PROVIDERS : MOMO_PROVIDERS).map((p) => (
                  <TouchableOpacity
                    key={p}
                    style={[
                      styles.providerChip,
